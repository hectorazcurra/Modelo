      ...
      {d.categorias?.length ? (
        <Section title="PPU por categoria">
          <table className="w-full text-xs">
            <thead>
              <tr className="text-[var(--fg-faint)] border-b border-[var(--border-base)]">
            ...
            <tbody>
              {(mode === 'full' ? d.categorias : d.categorias.slice(0, 8)).map((cat, i) =>
                cat.nome && (cat.total ?? 0) > 0 ? (
                  <tr key={i} className="border-b border-[var(--bg-elev)] last:border-0">
                    <td className="py-1 pr-3 text-[var(--fg-muted)]">{cat.nome}</td>
                    <td className="py-1 pr-3 text-right text-[var(--fg-base)]">{brl(cat.total ?? 0)}</td>
                    <td className="py-1 text-right text-[var(--fg-faint)]">{cat.itens?.length ?? 0}</td>
                  </tr>
                ) : null,
              )}
            </tbody>
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {(mode === 'full' ? d.equipes : d.equipes.slice(0, 8)).map((e, i) => {
                ...
                <Fragment key={i}>
                  <tr
                    className={`border-b border-[var(--bg-elev)] last:border-0 ${profs.length ? 'cursor-pointer hover:bg-[var(--bg-elev)]' : ''}`}
                    onClick={() => profs.length && setExpanded(isOpen ? null : i)}
                  >
                    <td className="py-1 pr-3 text-[var(--fg-muted)]">
                      <span className="inline-flex items-center gap-1">
                        {profs.length ? (
                          isOpen ? <ChevronDown className="w-3 h-3 text-[var(--fg-faint)]" /> : <ChevronRight className="w-3 h-3 text-[var(--fg-faint)]" />
                        ) : <span className="w-3 h-3 inline-block" />}
                        {e.nome ?? '—'}
                        {profs.length ? <span className="text-[var(--fg-faint)] text-[10px]">({profs.length})</span> : null}
                      </span>
                    </td>
                    <td className="py-1 pr-3 text-right text-[var(--fg-base)]">{Math.round(hh * 100) / 100}h</td>
                    <td className="py-1 pr-3 text-right text-[var(--fg-base)]">{brl(custo)}</td>
                    <td className="py-1 text-right text-[var(--fg-faint)]">{brl(custoPorH)}/h</td>
                  </tr>
                  {isOpen && profs.length ? (
                    <tr className="bg-[var(--bg-elev-strong)]">
                      <td colSpan={4} className="px-3 py-2">
                        <table className="w-full text-[11px]">
                          ...
                          <tbody>
                            {profs.map((p, j) => (
                              <tr key={j}>
                                <td className="py-0.5 pr-3 text-[var(--fg-muted)]">{p.funcao ?? '—'}</td>
                                <td className="py-0.5 pr-3 text-right text-[var(--fg-muted)]">{Math.round((p.hh ?? 0) * 100) / 100}h</td>
                                <td className="py-0.5 text-right text-[var(--fg-muted)]">{brl(p.custo ?? 0)}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
...
function MetaRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex gap-2">
      <span className="text-[var(--fg-faint)] flex-shrink-0">{label}:</span>
      <span className="text-[var(--fg-base)]">{value}</span>
    </div>
  )
}
...
function TextExcerpt({
  ...
  if (!collapsed) {
    return (
      <Section title={title}>
        <p className="text-xs text-[var(--fg-muted)] whitespace-pre-wrap leading-relaxed">{trimmed}</p>
      </Section>
    )
  }
  ...
  return (
    <details className="group">
      ...
      <p className="text-xs text-[var(--fg-muted)] whitespace-pre-wrap leading-relaxed mt-2">{trimmed}</p>
    </details>
  )
}
